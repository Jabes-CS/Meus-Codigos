...
def move_button_1(e):
    if abs(e.x - button_1.winfo_x()) < 50 and abs(e.y - button_1.winfo_y()) < 40:
        x = random.randint(0, root.wifo_width() - button_1.winfo_width())
        y = random.randint(0, root.wifo_height() - button_1.winfo_height())
        button_1.place(x = x, y = y)

def accepted():
    messagebox.showinfo(
        'Meu amor', 'Eu te amor, lanchinho mais tarde?')
